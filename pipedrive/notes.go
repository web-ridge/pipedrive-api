...
func (s *NotesService) listInner(ctx context.Context, start int) (*NotesResponse, *Response, error) {
	req, err := s.client.NewRequest(http.MethodGet, "/notes", PaginationOpts(start), nil)
	if err != nil {
		return nil, nil, err
	}
